...
void ShowSelectedNodeOrigin(Node@ node, int index)
{
    if (node !is null)
    {
        ...
        Vector2 sp = activeViewport.camera.WorldToScreenPos(node.worldPosition);
...
void MoveOrigin(int index, bool isVisible = false)
{
    ...
    Viewport@ vp = activeViewport.viewport;
    Vector2 sp = activeViewport.camera.WorldToScreenPos(originsNodes[index].worldPosition);
