// Dry editor view & camera functions
...
Ray GetActiveViewportCameraRay()
{
    ...
    return camera.GetScreenRay(Vector2((ui.cursorPosition.x - view.left) / view.width,
                                      (ui.cursorPosition.y - view.top) / view.height));
}
...
Vector3 GetNewNodePosition(bool raycastToMouse = false)
{
    ...
    {
        Ray cameraRay = raycastToMouse ? GetActiveViewportCameraRay() : camera.GetScreenRay(Vector2(0.5, 0.5));
...
{
    IntVector2 pos = ui.cursorPosition;
    Ray cameraRay = camera.GetScreenRay(Vector2(float(pos.x) / activeViewport.viewport.rect.width,
                                                float(pos.y) / activeViewport.viewport.rect.height));
