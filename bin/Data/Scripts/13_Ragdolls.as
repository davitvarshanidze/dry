...
void CreateScene()
{
    ...
    for (int z = -1; z <= 1; ++z)
    {
        for (int x = -4; x <= 4; ++x)
        {
            ...
            modelNode.position = Vector3(x * 3.0f, 0.0f, z * 4.0f);
            modelNode.rotation = Quaternion(0.0f, 180.0f, 0.0f);
            AnimatedModel@ modelObject = modelNode.CreateComponent("AnimatedModel");
            modelObject.model = cache.GetResource("Model", "Models/Robo.mdl");
            modelObject.material = cache.GetResource("Material", "Materials/Chrome.xml");
...
class CreateRagdoll : ScriptObject
{
    ...
    void HandleNodeCollision(StringHash eventType, VariantMap& eventData)
    {
        ...
        if (otherBody.mass > 0.0f)
        {
            ...
            CopyPhysicsComponents(cache.GetResource("XMLFile", "Objects/Robo.xml"), node);
/*
...
*/
    ...
    {
        XMLElement rootElem = prefab.GetRoot();

        
    }

    void CopyPhysicsComponents(const XMLElement& from, Node@ to)
    {
