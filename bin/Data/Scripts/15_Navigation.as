...
void CreateScene()
{
    ...
    planeObject.material = cache.GetResource("Material", "Materials/MudLeavesTiled.xml");
    ...
    for (uint i = 0; i < NUM_BOXES; ++i)
    {
        ...
        boxObject.model = cache.GetResource("Model", "Models/FancyBox.mdl");
        boxObject.material = cache.GetResource("Material", "Materials/GrassRock.xml");
    ...
    ozomNode = scene_.CreateChild("Ozom");
    ozomNode.position = Vector3(-5.0f, 0.0f, 20.0f);
    ozomNode.Rotate(Quaternion(0.0f, 160.0f, 0.0f));
    AnimatedModel@ modelObject = ozomNode.CreateComponent("AnimatedModel");
    modelObject.model = cache.GetResource("Model", "Ghotiland/Models/Ozom.mdl");
    modelObject.material = cache.GetResource("Material", "Ghotiland/Materials/Ozom.xml");
    ...
    cameraNode.position = Vector3(0.0f, 42.0f, 0.0f);
    pitch = 70.0f;
...
void MoveCamera(float timeStep)
{
    ...
    const Vector3 forward = cameraNode.direction.ProjectOntoPlane(Vector3::UP, Vector3::ZERO).Normalized();

    // Read WASDEQ keys and move the camera scene node to the corresponding direction if they are pressed
    if (input.keyDown[KEY_W])
        cameraNode.Translate(forward * MOVE_SPEED * timeStep, TS_WORLD);
...
void SetPathPoint()
{
    ...
    if (Raycast(250.0f, hitPos, hitDrawable))
    {
        ...
        if (input.qualifierDown[QUAL_SHIFT])
        {
            // Teleport
            currentPath.Clear();
            ozomNode.LookAt(Vector3(pathPos.x, ozomNode.position.y, pathPos.z), Vector3::UP);
            ozomNode.position = pathPos;
        }
        else
        {
            // Calculate path from Ozom's current position to the end point
            endPos = pathPos;
            currentPath = navMesh.FindPath(ozomNode.position, endPos);
        }
    }
}
...
Node@ CreateMushroom(const Vector3& pos)
{
    ...
    mushroomNode.scale = Vector3::ONE * (2.0f + Random(3.f)) + Vector3::UP * 2.f;
    StaticModel@ mushroomObject = mushroomNode.CreateComponent("StaticModel");
    mushroomObject.model = cache.GetResource("Model", "Models/Mushroom.mdl");
    mushroomObject.material = cache.GetResource("Material", "Materials/Fire.xml");
...
void FollowPath(float timeStep)
{
    if (currentPath.length > 0)
    {
        ...
        float distance = (ozomNode.position - nextWaypoint).length;
        if (move > distance)
            move = distance;

        ozomNode.LookAt(nextWaypoint, Vector3::UP);
        ozomNode.Translate(Vector3::FORWARD * move);
...
void UpdateStreaming()
{
    ...
    IntVector2 ozomTile = navMesh.GetTileIndex(ozomNode.worldPosition);
    IntVector2 beginTile = VectorMax(IntVector2(0, 0), ozomTile - IntVector2(1, 1) * STREAMING_DISTANCE);
    IntVector2 endTile = VectorMin(ozomTile + IntVector2(1, 1) * STREAMING_DISTANCE, navMesh.numTiles - IntVector2(1, 1));
...
void HandlePostRenderUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (currentPath.length > 0)
    {
        ...
        debug.AddLine(ozomNode.position + bias, currentPath[0] + bias, Color(1.0f, 1.0f, 1.0f));
