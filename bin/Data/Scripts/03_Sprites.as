...
void CreateSprites()
{
    ...
    Texture2D@ decalTex = cache.GetResource("Texture2D", "Textures/DryDecal.png");
...
void MoveSprites(float timeStep)
{
    ...
    for (uint i = 0; i < sprites.length; ++i)
    {
        ...
        // Rotate
        float newRot = sprite.rotation + timeStep * (120.0f + (i % 5) * 6.0f);
        sprite.rotation = newRot;

        // Move, wrap around rendering window edges
        Vector2 newPos = sprite.position + sprite.vars["Velocity"].GetVector2() * timeStep + Vector2::DOWN * 0.5f;
        if (newPos.x < 0.0f)
            newPos.x += width;
        if (newPos.x >= width)
            newPos.x -= width;
