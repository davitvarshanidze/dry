// Convenient functions for 2D samples:
...
Node@ CreateEnemy()
{
    ...
    staticSprite.sprite = cache.GetResource("Sprite2D", "2D/Aster.png");
    RigidBody2D@ body = node.CreateComponent("RigidBody2D");
    body.bodyType = BT_STATIC;
...
Node@ CreateOrc()
{
    ...
    AnimationSet2D@ spriterAnimationSet = cache.GetResource("AnimationSet2D", "2D/Orc/Orc.scml");
    if (spriterAnimationSet is null)
        return null;
...
Node@ CreateCoin()
{
    ...
    AnimationSet2D@ spriterAnimationSet = cache.GetResource("AnimationSet2D", "2D/GoldIcon.scml");
    if (spriterAnimationSet is null)
        return null;
...
Node@ CreateMovingPlatform()
{
    ...
    StaticSprite2D@ staticSprite = node.CreateComponent("StaticSprite2D");
    staticSprite.sprite = cache.GetResource("Sprite2D", "2D/Box.png");
    RigidBody2D@ body = node.CreateComponent("RigidBody2D");
    body.bodyType = BT_STATIC;
    CollisionBox2D@ shape = node.CreateComponent("CollisionBox2D"); // Create box shape
...
void CreateUIContent(String demoTitle)
{
    ...
    coinsUI.texture = cache.GetResource("Texture2D", "2D/GoldIcon.png");
    ...
    lifeUI.texture = cache.GetResource("Texture2D", "2D/imp/imp_all.png");
    ...
    spriteUI.texture = cache.GetResource("Texture2D", "2D/imp/imp_all.png");
...
void SpawnEffect(Node@ node)
{
    ...
    particleEmitter.effect = cache.GetResource("ParticleEffect2D", "2D/sun.pex");
