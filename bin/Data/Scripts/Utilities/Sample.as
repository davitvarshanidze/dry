...
void CreateLogo()
{
    ...
    const int textureWidth = logoTexture.width;
    const int textureHeight = logoTexture.height;
    ...
    logoSprite.SetHotSpot(textureWidth + 4, textureHeight + 2);

    // Set logo sprite alignment
    logoSprite.SetAlignment(HA_RIGHT, VA_BOTTOM);

    // Make logo not fully opaque to show the scene underneath
    logoSprite.opacity = 0.95f;
...
void SetWindowTitleAndIcon()
{
    ...
    graphics.windowTitle = "Dry Sample";
}
...
void CreateInstructions(const String& text)
{
    // Construct new Text object, set string to display and font to use
    Text@ instructionText = ui.root.CreateChild("Text");
    instructionText.text = text;
    instructionText.SetFont(cache.GetResource("Font", "Fonts/Philosopher.ttf"), 16);
    instructionText.color = GRAY * 1.8f;
    instructionText.effectColor = BLACK * 0.3f;
    instructionText.textEffect = TE_STROKE;
    ...
    instructionText.SetPosition(ui.root.height / 64, ui.root.height / 64);
}
...
{
    Camera@ camera = cameraNode.GetComponent("Camera");
    Vector3 worldPos = camera.ScreenToWorldPos(graphics.NormalizedScreenPos(input.mousePosition), 10.0f);
    return VectorTo2D(worldPos);
}
