...
void PaintDecal()
{
    ...
    if (Raycast(250.0f, hitPos, hitDrawable))
    {
        ...
        if (decal is null)
        {
            ...
            decal.material = cache.GetResource("Material", "Materials/DryDecalAlpha.xml");
        }
        // Add a square decal to the decal set using the geometry of the drawable that was hit, orient it to face the camera,
        // use full texture UV's (0,0) to (1,1). Note that if we create several decals to a large object (such as the ground
        // plane) over a large area using just one DecalSet component, the decals will all be culled as one unit. If that is
        // undesirable, it may be necessary to create more than one DecalSet based on the distance
        Quaternion decalRotation = Quaternion(Random(360.0f), cameraNode.direction) * cameraNode.rotation;
        decal.AddDecal(hitDrawable, hitPos, decalRotation, 1.0f, 1.0f, 1.0f, Vector2::ZERO, Vector2::ONE);
    }
}
