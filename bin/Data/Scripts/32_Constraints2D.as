...
void CreateScene()
{
    ...
    camera.zoom = Min(graphics.width / 1280.0f, graphics.height / 800.0f); // Set zoom according to user's resolution to ensure full visibility (initial zoom (1.2) is set for full visibility at 1280x800 resolution)
    ...
    boxSprite.sprite = cache.GetResource("Sprite2D", "2D/Box.png");
    ...
    ballSprite.sprite = cache.GetResource("Sprite2D", "2D/Ball.png");
    ...
    polygonSprite.sprite = cache.GetResource("Sprite2D", "2D/Aster.png");
...
void MoveCamera(float timeStep)
{
    ...
    if (input.keyDown[KEY_Q]) camera.zoom = camera.zoom * 1.01f; // Zoom In
    if (input.keyDown[KEY_E]) camera.zoom = camera.zoom * 0.99f; // Zoom Out
}
...
{
    PhysicsWorld2D@ physicsWorld = scene_.GetComponent("PhysicsWorld2D");
    RigidBody2D@ rigidBody = physicsWorld.GetRigidBodyAtScreenPoint(input.mousePosition); // Raycast for RigidBody2Ds to pick
...
{
    PhysicsWorld2D@ physicsWorld = scene_.GetComponent("PhysicsWorld2D");
    IntVector2 screenPoint(eventData["X"].GetInt(), eventData["Y"].GetInt());
    RigidBody2D@ rigidBody = physicsWorld.GetRigidBodyAtScreenPoint(screenPoint); // Raycast for RigidBody2Ds to pick
    ...
    if (rigidBody !is null)
    {
        ...
        ConstraintMouse2D@ constraintMouse = pickedNode.CreateComponent("ConstraintMouse2D");
        IntVector2 screenPoint(eventData["X"].GetInt(), eventData["Y"].GetInt());
        Vector2 pos = VectorTo2D(camera.ScreenToWorldPos(graphics.NormalizedScreenPos(screenPoint), 0.0f));
        constraintMouse.target = pos;
        constraintMouse.maxForce = 1000 * rigidBody.mass;
        constraintMouse.collideConnected = true;
        constraintMouse.otherBody = dummyBody;  // Use dummy body instead of rigidBody. It's better to create a dummy body automatically in ConstraintMouse2D
