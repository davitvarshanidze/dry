...
void CreateScene()
{
    ...
    SpawnOzom(Vector3(-5.0f, 0, 20.0f), scene_.CreateChild("Ozoms"));
...
void CreateUI()
{
    ...
    CreateInstructions("Use WASDQE keys to move, RMB to rotate view\n"
                       "LMB to set destination, Shift+LMB to spawn Ozom\n"
...
void SpawnOzom(const Vector3& pos, Node@ ozomGroup)
{
    Node@ ozomNode = ozomGroup.CreateChild("Ozom");
    ozomNode.position = pos;
...
void SetPathPoint(bool spawning)
{
    ...
    if (Raycast(250.0f, hitPos, hitDrawable))
    {
        ...
        if (spawning)
            ...
            cast<CrowdManager>(scene_.GetComponent("CrowdManager")).SetCrowdTarget(pathPos, ozomGroup);
    }
}
...
void MoveCamera(float timeStep)
{
    ...
    // Set destination or spawn a ozom with left mouse button
...
void UpdateStreaming()
{
    ...
    Node@ ozomGroup = scene_.GetChild("Ozoms");
    if (ozomGroup !is null)
    {
        for (uint i = 0; i < ozomGroup.numChildren; ++i)
            averageOzomPosition += ozomGroup.children[i].worldPosition;
        averageOzomPosition /= ozomGroup.numChildren;
    }

    // Compute currently loaded area
    IntVector2 ozomTile = navMesh.GetTileIndex(averageOzomPosition);
...
{
    const String WALKING_ANI = "Ghotiland/Anim/Ozom/Walk.ani";
