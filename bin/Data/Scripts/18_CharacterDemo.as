...
void Start()
{
    ...
    CreateInstructions("Use WASD keys and mouse to move\n"
...
void CreateScene()
{
    ...
    zone.ambientColor = Color(0.2f, 0.23f, 0.27f);
    ...
    floor.model = cache.GetResource("Model", "Models/Box.mdl");
    floor.material = cache.GetResource("Material", "Materials/MudLeavesTiled.xml");

    RigidBody@ floorBody = floorNode.CreateComponent("RigidBody");
    // Use collision layer bit 2 to mark world scenery. This is what we will raycast against to prevent camera from going
    // inside geometry
    floorBody.collisionLayer = 2;
    CollisionShape@ boxShape = floorNode.CreateComponent("CollisionShape");
    boxShape.SetBox(Vector3::ONE);

    // Create mushrooms of varying sizes
    const uint NUM_MUSHROOMS = 90;
    for (uint i = 0; i < NUM_MUSHROOMS; ++i)
    {
        ...
        objectNode.SetScale(RandomNormal(6.0f, 3.0f));
        StaticModel@ object = objectNode.CreateComponent("StaticModel");
        object.model = cache.GetResource("Model", "Models/Mushroom.mdl");
        object.material = cache.GetResource("Material", "Materials/Mushroom.xml");
        object.castShadows = true;

        RigidBody@ fruitBody = objectNode.CreateComponent("RigidBody");
        fruitBody.collisionLayer = 2;
        CollisionShape@ mushape = objectNode.CreateComponent("CollisionShape");
        mushape.SetTriangleMesh(object.model, 0);
    }
    ...
    {
        float scale = RandomNormal(2.3f, 0.7f);

        Node@ objectNode = scene_.CreateChild("Box");
        objectNode.position = Vector3(RandomNormal(0.0f, 100.0f), Random(20.0f) + 10.0f, Random(60.0f) + 10.0f);
        objectNode.rotation = Quaternion(Random(360.0f), Random(360.0f), Random(360.0f));
        objectNode.SetScale(scale);
        StaticModel@ object = objectNode.CreateComponent("StaticModel");
        object.model = cache.GetResource("Model", "Models/FancyBox.mdl");
        object.material = cache.GetResource("Material", "Materials/GrassRock.xml");
        object.castShadows = true;

        RigidBody@ boxBody = objectNode.CreateComponent("RigidBody");
        boxBody.collisionLayer = 2;
        // Bigger boxes will be heavier and harder to move
        boxBody.mass = scale * scale * scale * 1.5f;
...
{
    characterNode = scene_.CreateChild("Ozom");

    // Create the rendering component + animation controller
    AnimatedModel@ object = characterNode.CreateComponent("AnimatedModel");
    object.model = cache.GetResource("Model", "Ghotiland/Models/Ozom.mdl");
    object.material = cache.GetResource("Material", "Ghotiland/Materials/Ozom.xml");
    object.castShadows = true;
    characterNode.CreateComponent("AnimationController");

    // Set the head bone for manual control
    object.skeleton.GetBone("Head").animated = false;
...
void HandleUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (ui.focusElement is null)
    {
        ...
        if (input.keyPress[KEY_F7])
        {
            ...
            characterNode = scene_.GetChild("Ozom", true);
            if (characterNode is null)
                return;
        }
    }
...
void HandlePostUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    Vector3 headWorldTarget = headNode.worldPosition + headDir * Vector3::FORWARD;
