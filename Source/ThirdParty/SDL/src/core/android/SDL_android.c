...
JNIEXPORT int JNICALL SDL_JAVA_INTERFACE(nativeRunMain)(JNIEnv *env, jclass cls, jstring library, jstring function, jobject array)
{
    ...
    if (library_handle) {
        ...
        if (SDL_main) {
            ...
            // Dry: Remove any assumption on the arguments, except that the first argument will be the program name set by SDLActivity or its subclass
                                    ...
                                    JNIEnv *env, jclass cls)
{
    ...
    // Dry: Free the memory that we allocate during init
    if (mFilesDir) {
        free(mFilesDir);
        mFilesDir = 0;
    }
...
// Dry - function to return a list of files under a given path in "assets" directory (caller is responsible to free the C string array)
...
// Dry - helper function to free the file list returned by SDL_Android_GetFileList()
