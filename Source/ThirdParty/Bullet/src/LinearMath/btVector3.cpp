 ...
 */
 ...
 // Modified by Yao Wei Tjong & Lasse Oorni for Dry
...
#if defined BT_USE_SSE //|| defined _WIN32      // Dry - just use BT_USE_SSE as the main switch, Dry allow SSE to be disabled
...
long _maxdot_large( const float *vv, const float *vec, unsigned long count, float *dotResult )
{
    ...
    // Dry: cast to float in case BT_INFINITY is double on some MinGW derived compilers to prevent narrowing errors
...
#include <arm_neon.h>
#include <sys/types.h>
// Dry - enable NEON on generic ARM
#ifdef __APPLE__
