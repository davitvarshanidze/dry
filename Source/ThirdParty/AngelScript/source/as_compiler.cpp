...
// Modified by Lasse Oorni for Dry
...
int asCCompiler::DoAssignment(asCExprContext *ctx, asCExprContext *lctx, asCExprContext *rctx, asCScriptNode *lexpr, asCScriptNode *rexpr, eTokenType op, asCScriptNode *opNode)
{
    ...
    // Dry: if there is a handle type, and it does not have an overloaded assignment operator, convert to an explicit handle
    // for scripting convenience. (For the Dry handle types, value assignment is not supported)
