...
// Modified by Lasse Oorni, Yao Wei Tjong, 1vanK and cosmy1 for Dry
...
dtCrowd::dtCrowd() :
	m_updateCallback(0), // Dry: Add update callback support
	m_maxAgents(0),
	m_agents(0),
	m_activeAgents(0),
	m_agentAnims(0),
	...
	m_navquery(0)
{
	// Dry: initialize all class members
...
// Dry: Add update callback support
/// @par
///
...
int dtCrowd::addAgent(const float* pos, const dtCrowdAgentParams* params)
{
	...
	// Dry: added to fix illegal memory access when ncorners is queried before the agent has updated
	ag->ncorners = 0;
	...
	for (int iter = 0; iter < 4; ++iter)
	{
		for (int i = 0; i < nagents; ++i)
		{
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				// Dry: Avoid tremble when another agent can not move away
